...
## 数据迁移与安装生命周期（Issue #10）

SQLite schema 当前版本为 **v3**。首次安装自动建立 v1，再按顺序应用 v2、v3；每个版本在独立事务中完成，失败则回滚当前版本并阻止后端 HTTP 路由注册。错误只指出失败的迁移版本，不把 SQL、数据路径、配置或提示词输出到普通日志。v3 为资产来源、任务来源和任务结果建立外键（删除引用资产时 `SET NULL`），并保留资产分页、收藏和来源查询索引。重复启动已完成版本不会重建表；升级 v1/v2 时保留任务、图片元数据及图片文件。迁移前请备份持久化数据目录，**不要**在失败后手工修改 `schema_version` 来跳过迁移，应先检查备份、磁盘空间、文件权限和旧库的一致性。

数据目录由 `IMAGENIA_DATA_DIR` 指定；未指定时，历史默认仍为 `~/.qwenpaw/plugins/imagenia`，以免升级时静默改用空目录。**容器与正式安装建议明确设置该变量，指向独立于插件代码目录的持久化 volume**；升级、覆盖安装及重装必须使用相同的变量值、挂载和文件属主。插件停用会停止领取新任务，运行中任务标记 `failed/interrupted` 且不会自动重试；已有 `pending` 在下次启用时恢复。插件自身的 `unregister()` **不会删除**密钥配置、SQLite 或 PNG，重新注册同一数据目录可继续使用。卸载由宿主管理插件代码/路由；插件不主动清理数据，**但若宿主卸载时删除了包含默认数据目录的整个插件目录，数据也可能随之丢失**。因此卸载前先备份并核对持久化 volume；需要彻底删除数据时由管理员在卸载和备份确认后单独清理，不自动执行不可逆操作。

本地回归 `python -m pytest plugin/tests -q` 覆盖首次安装、v1/v2 升级、幂等重启、迁移回滚与阻止注册、约束/外键/索引、配置/任务/收藏/图片保留、模拟停用和重装；不模拟 QwenPaw 的真实卸载文件策略。真机验收请在测试 volume 上备份后，核对覆盖安装、重启、停用与重装前后的配置状态、任务、收藏、图片可读性和文件权限，并确认卸载是否会连带删除默认目录，再决定生产数据路径。
